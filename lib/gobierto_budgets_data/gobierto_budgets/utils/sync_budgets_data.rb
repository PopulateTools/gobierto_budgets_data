    ...
    module Utils
      class SyncBudgetsData
        def initialize(from_url, to_url, ine_code)
          @from = Elasticsearch::Client.new log: true, url: from_url
          @to = Elasticsearch::Client.new log: true, url: to_url
          @ine_code = ine_code
        end
        ...
        def sync(year)
          [
            ...
            [
              ...
              terms = [{term: { ine_code: @ine_code } }, { term: { year: year } }, { term: { type: type }} ]
              results = @from.search(index: index, body: { query: { filtered: { filter: { bool: { must: terms }}}}, size: 10_000 })
